...
def main():
    ...
    for v_model_dir in subfolders:
        ...
        for offline_algorithm in OFFLINE_ALGORITHMS:

            try:
                v_offline_model_dir = path_join(v_offline_models_dir, offline_algorithm.lower())
                v_offline_model_json_dir = get_subdir(v_offline_model_dir)
                v_offline_model_json_file_name = path_join(v_offline_model_json_dir, 'params.json')
                shutil.copy(v_offline_model_json_file_name, v_offline_model_dir)
                shutil.rmtree(v_offline_model_json_dir)
