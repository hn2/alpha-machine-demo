...
def run_trial(online_algorithm, train_look_back_period, total_timesteps, market, resolution, num_of_instruments, spread,
              subdir, train_test_split, config_file):
    ...
    v_callback_lookback, v_save_freq, v_ppo_params, v_td3_params, v_sac_params, v_net_arch, v_use_linear_schedule, v_action_noise, v_noise_sigma, v_use_sde = get_agent_params(
        config_file)
...
def main():
    ...
    v_algorithms, v_number_of_trials, v_subdir, v_train_test_split, v_train_look_back_period, v_total_timesteps, v_market, v_resolution, v_num_of_instruments, v_spread = get_train_params(
        v_config_file)

    for i in range(v_number_of_trials):
        run_trial(v_algorithms, v_train_look_back_period, v_total_timesteps, v_market, v_resolution,
                  v_num_of_instruments, v_spread, v_subdir, v_train_test_split, v_config_file)
