...
from findrl.config_utils import get_paths_params, get_data_params, get_pf_fx_env_params, get_train_params, \
    get_agent_params, get_model_params
from findrl.data_utils import prepare_date_train
from findrl.env_utils import make_env
...
def run_trial(online_algorithm, train_look_back_period, total_timesteps, config_file):
    ...
    v_num_of_instruments = int(random.choice(v_num_of_instruments))
    ...
    v_callback_lookback, v_save_freq, v_ppo_params, v_td3_params, v_net_arch, v_use_linear_schedule, v_action_noise, v_noise_sigma, v_use_sde = get_agent_params(
        config_file)
...
def main():
    v_config_file = 'config-fxcm.ini'

    v_algorithms, v_number_of_trials, v_train_look_back_period, v_total_timesteps = get_train_params(
        v_config_file)

    for i in range(v_number_of_trials):
        run_trial(random.choice(v_algorithms), v_train_look_back_period, v_total_timesteps, v_config_file)
