...
CONFIG_FILE = '../settings/train/config-train-oanda-daily-a2c.ini'
...
def main():
    ...
    v_algorithms, v_model_verbose, v_callback_verbose, v_save_replay_buffer, v_use_tensorboard, v_use_callback, v_check_freq, \
    v_callback_lookback, v_save_freq, v_params, v_learning_rate, v_batch_sizes, v_net_arch, v_use_linear_schedule, v_action_noises, v_noise_sigma, v_use_sde = get_agent_params_sb3(
        CONFIG_FILE)
    ...
    v_compute_position, v_compute_indicators, v_compute_reward, v_meta_rl, v_env_verbose, v_num_of_envs = get_pf_fx_env_params(
        CONFIG_FILE)
    v_number_of_trials, v_subdir, v_train_look_back_period, v_total_timesteps, v_log_interval, v_market, \
    ...
    for i in range(v_number_of_trials):
        ...
        v_online_algorithm = random.choice(v_algorithms)
        ...
        if v_online_algorithm == 'PPO' or v_online_algorithm == 'DDPG' or v_online_algorithm == 'DQN' or v_online_algorithm == 'SAC' or v_online_algorithm == 'TD3':
            ...
            v_model_prefix_full = f'{v_model_prefix}_week_{str(WEEK)}_batch_{str(batch_size)}_'
        elif v_online_algorithm == 'A2C':
            n_steps = v_params['n_steps']
            v_model_prefix_full = f'{v_model_prefix}_week_{str(WEEK)}_steps_{str(n_steps)}_'
