...
CONFIG_FILE = '../settings/config-oanda-train-ppo.ini'
NUM_CPU = 2


def make_env(online_algorithm, train_look_back_period, total_timesteps, market, resolution, num_of_instruments,
...
def run_trial(env, instruments, online_algorithm, train_look_back_period, total_timesteps, market, resolution,
    ...
    # v_env = SubprocVecEnv([make_env(env_id=env_id, data=df_train, mode=MODE, method=METHOD, lookback=25,
    #                                 features_selection_method=FEATURES_SELECTION_METHOD, random_episode_start=True,
    #                                 meta_rl=False, verbose=False, rank=i, seed=0) for i in range(NUM_CPU)])
    #   v_env = SubprocVecEnv([get_env_train(data=df_train, mode=MODE, method=METHOD, lookback=25, features_selection_method=FEATURES_SELECTION_METHOD, random_episode_start=True, meta_rl=False, verbose=False) for i in range(NUM_CPU)])
    ...
    if v_online_algorithm == 'SAC' or v_online_algorithm == 'DDPG' or v_online_algorithm == 'TD3':
        v_online_model = v_online_class(env=env, policy=v_online_policy, **v_params,
...
def main():
    ...
    v_env = SubprocVecEnv([make_env(i) for i in range(NUM_CPU)])
    v_env = VecNormalize(v_env, norm_obs=True, norm_reward=True)
