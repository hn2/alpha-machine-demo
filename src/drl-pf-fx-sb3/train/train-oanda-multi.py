...
def make_env(env, rank, seed=0) -> Callable:
    """
...
def run_trial(online_algorithm, train_look_back_period, total_timesteps, market, resolution, num_of_instruments, spread,
    ...
    # v_subproc_vec_env = SubprocVecEnv([make_env(v_dummy_vec_env, i) for i in range(num_cpu)])
    v_subproc_vec_env = make_vec_env(v_env, n_envs=num_cpu)
    ...
    if v_online_algorithm == 'PPO' or v_online_algorithm == 'A2C' or v_online_algorithm == 'SAC':
        v_online_model = online_class(env=v_subproc_vec_env, policy=online_policy,
...
def main():
    v_config_file = 'settings/config-oanda.ini'
    ...
    for i in range(v_number_of_trials):
        run_trial(v_algorithms, v_train_look_back_period, v_total_timesteps, v_market, v_resolution,
                  v_num_of_instruments, v_spread, v_num_cpu, v_subdir, v_train_test_split, v_config_file)
