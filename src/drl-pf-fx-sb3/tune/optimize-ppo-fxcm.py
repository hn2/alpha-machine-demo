...
MARKET = 'fxcm'  # oanda   fxcm
RESOLUTION = 'daily'  # day  hour
COMPUTE_REWARD = ['log_returns']
ONLINE_ALGORITHM = 'PPO'  # PPO A2C TD3 SAC
...
class optimise():
    ...
    def optimize_env(self, trial):
        ...
        return {
            ...
            'number_of_instruments': trial.suggest_categorical('number_of_instruments', [7]),
    ...
    def optimize_model(self, trial):
        ...
        batch_size = trial.suggest_discrete_uniform('batch_size', 256, 2048, 256)
        n_steps = trial.suggest_discrete_uniform('n_steps', 256, 2048, 256)
        n_epochs = trial.suggest_discrete_uniform('n_epochs', 5, 30, 5)
        clip_range = trial.suggest_discrete_uniform('clip_range', 0.1, 0.3, 0.1)
    ...
    def get_online_class_and_policy(self, algorithm):
        ...
        if algorithm == 'PPO':
            online_class = PPO
            online_policy = PpoMlpPolicy
        elif algorithm == 'A2C':
            online_class = A2C
            online_policy = A2cMlpPolicy
        elif algorithm == 'TD3':
            online_class = TD3
    ...
    def optimize_agent(self, trial):
        ...
        if CREATE_TENSORBOARD_LOG:
            ...
            online_model = online_class(env=v_vec_normalize, policy=online_policy, verbose=VERBOSE,
                                        **model_params, tensorboard_log=OPTUNA_LOGS_DIR)
