...
from findrl.SaveInMemoryOnBestTrainingRewardCallback import SaveInMemoryOnBestTrainingRewardCallback
from findrl.config_utils import get_paths_params, get_optuna_params, get_pf_fx_env_params
...
from findrl.model_utils import get_online_class
from findrl.optuna_sample_params_4 import HYPERPARAMS_SAMPLER
...
N_EVAL_EPISODES = 3
N_JOBS = 6
STUDY_NAME_PREFIX = 'fx14-leverage-30'
...
def get_envs():
    ...
    v_dummy_vec_env_train = DummyVecEnv([lambda: Monitor(v_env_train, v_logs_dir)])
    v_dummy_vec_env_train.seed(1)
    v_vec_normalize_train = VecNormalize(v_dummy_vec_env_train)
    v_vec_normalize_train.seed(1)

    v_dummy_vec_env_eval = DummyVecEnv([lambda: Monitor(v_env_train, v_logs_dir)])
    v_dummy_vec_env_eval.seed(1)
    v_vec_normalize_eval = VecNormalize(v_dummy_vec_env_train)
    v_vec_normalize_eval.seed(1)

    return v_vec_normalize_train, v_vec_normalize_eval
...
def objective(trial: optuna.Trial) -> float:
    ...
    model = get_online_class(algo)(**kwargs)
    eval_env = v_env_eval
    ...
    SaveInMemoryOnBestTrainingReward = SaveInMemoryOnBestTrainingRewardCallback(check_freq=100, lookback=100,
                                                                                verbose=False)

    try:
        model.learn(N_TIMESTEPS, callback=[eval_callback, SaveInMemoryOnBestTrainingReward])
