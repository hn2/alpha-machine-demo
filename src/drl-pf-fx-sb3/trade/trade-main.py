...
from findrl.general_utils import upload_to_dropbox
...
class main(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.Debug(self.algorithms)
        self.Debug(self.sort_columns)
        self.Debug(self.stat_column)
        self.Debug(self.position)
        self.Debug(self.number)
        self.Debug(str(self.upload_dropbox))
        self.Debug(str(self.trade_oanda))
        self.Debug(str(self.rebalance_oanda))
        self.Debug(str(self.trade_fxcm))
        self.Debug(str(self.rebalance_fxcm))
        self.Debug(str(self.liquidate_fxcm))
        self.Debug(self.dropbox_file_name)
        self.Debug(self.dropbox_remote_dir)
    ...
    def rebalance(self):
        ...
        v_suggested_weights_all = np.empty(shape=(len(self.model_names), len(self.instruments) + 1), dtype=np.float)
        ...
        for i, model_name in enumerate(self.model_names):
            ...
            v_suggested_weights_all[i] = v_suggested_weights
            #   suggested_positions[i] = v_vec_normalize.get_attr('last_suggested_positions')[0]

        v_weights = np.average(v_suggested_weights_all, axis=0, weights=self.stats)
